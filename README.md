...
Enable the `geodetic` feature for a great-circle spatial index over longitude/latitude data. `GeodeticRTree` embeds each `(lon, lat)` point on the unit sphere, so nearest-neighbour, radius and window queries use great-circle distance (in metres) instead of the misleading Euclidean-in-degrees distance, and the ±180° antimeridian and the poles are handled natively. See the [crate documentation](https://docs.rs/rstar/).
