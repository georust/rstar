...
//! End-to-end integration tests for [`GeodeticRTree`].
//!
...
use rstar::geodetic::{
    haversine_distance, GeodeticCoord, GeodeticPoint, GeodeticRTree, EARTH_RADIUS_METRES,
};
...
fn assert_matches_brute_force(points: &[GeodeticPoint], query: GeodeticCoord, radius_metres: f64) {
    let tree = GeodeticRTree::bulk_load(points.to_vec());
...
fn antipodal_distance_is_half_circumference() {
    let tree = GeodeticRTree::bulk_load(vec![GeodeticPoint::new(0.0, 0.0)]);
...
fn coincident_and_empty_cases() {
    ...
    // Empty tree. Annotate the leaf type: the default type parameter applies in type
    // position, not in the bare `GeodeticRTree::new()` method call.
    let empty: GeodeticRTree = GeodeticRTree::new();
...
fn locate_all_at_point_returns_every_duplicate() {
    ...
    let tree = GeodeticRTree::bulk_load(vec![dup, dup, dup, other]);
...
fn remove_at_point_removes_a_present_coord_and_shrinks() {
    let mut tree = GeodeticRTree::bulk_load(vec![
        GeodeticPoint::new(10.0, 20.0),
...
fn seam_straddling_cluster() {
    ...
    let tree = GeodeticRTree::bulk_load(cluster.clone());
...
/// The serde derives compose: a populated tree round-trips through JSON with its
/// leaves (original degrees and precomputed embedding) intact.
#[cfg(feature = "serde")]
#[test]
fn tree_serde_round_trip() {
    let points = vec![
        GeodeticPoint::new(-0.1278, 51.5074),
        GeodeticPoint::new(179.0, -17.0),
        GeodeticPoint::new(0.0, 90.0),
    ];
    let tree = GeodeticRTree::bulk_load(points);
    let json = serde_json::to_string(&tree).expect("serialise");
    let back: GeodeticRTree = serde_json::from_str(&json).expect("deserialise");
    assert_eq!(back.size(), tree.size());
    let q = coord(1.0, 51.0);
    assert_eq!(
        back.nearest_neighbor(q).map(|p| p.coord()),
        tree.nearest_neighbor(q).map(|p| p.coord())
    );
}

// ---------------------------------------------------------------------------
// Degenerate inputs: non-finite queries, negative radii, seam/pole spellings
// ---------------------------------------------------------------------------
...
/// Exact-location lookups agree with the window query about coordinate
/// identity: the canonicalised embedding makes ±180 the same meridian and any
/// pole spelling the same point.
#[test]
fn locate_at_point_matches_seam_and_pole_spellings() {
    let mut tree = GeodeticRTree::bulk_load(vec![
        GeodeticPoint::new(180.0, 0.0),
        GeodeticPoint::new(45.0, 90.0),
    ]);

    // Seam: stored at +180, found (and removable) under either spelling.
    assert!(tree.locate_at_point(coord(-180.0, 0.0)).is_some());
    assert!(tree.locate_at_point(coord(180.0, 0.0)).is_some());

    // Pole: longitude is undefined, so any spelling matches.
    assert!(tree.locate_at_point(coord(0.0, 90.0)).is_some());
    assert!(tree.locate_at_point(coord(-137.0, 90.0)).is_some());

    let removed = tree.remove_at_point(coord(-180.0, 0.0));
    assert_eq!(removed.map(|p| p.coord()), Some(coord(180.0, 0.0)));
}
