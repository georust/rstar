use crate::{PointDistance, RTreeObject, AABB};
...
// Serialised as the bare coordinate: the embedding is re-derived on deserialise
// via `From<GeodeticCoord>`, so a payload whose coordinate and vector disagree
// (a state no constructor can produce) is unrepresentable on the wire.
#[cfg_attr(
    feature = "serde",
    serde(from = "GeodeticCoord", into = "GeodeticCoord")
)]
...
impl GeodeticPoint {
    ...
    /// An out-of-range coordinate is embedded as-is – the trigonometric embedding
    /// effectively wraps it onto the sphere, so distance queries may still find the
    /// point – but the window query
    /// ([`GeodeticRTree::locate_in_rectangle`](super::GeodeticRTree::locate_in_rectangle))
    /// tests the *stored* degrees and will not return it.
    ///
...
/// Extracts the stored `(lon, lat)`; the inverse of `From<GeodeticCoord>`.
impl From<GeodeticPoint> for GeodeticCoord {
...
mod tests {
    ...
    use crate::geodetic::embedding::{squared_chord, UnitVec};
