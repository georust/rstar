...
///
/// A non-finite coordinate component propagates: the result is NaN rather than
...
pub fn haversine_distance(a: GeodeticCoord, b: GeodeticCoord) -> f64 {
    ...
    // making `sqrt(inner) > 1` and `asin(>1) = NaN`).
    let c = 2.0 * super::clamp_unit(inner.sqrt()).asin();
...
/// drift past `1.0`.
///
/// Query methods already return metres, so this is only needed to reason about the raw
/// metric directly.
///
/// Out-of-contract inputs clamp into the metric's range: a negative or NaN input
/// maps to `0.0` metres, and an input above `4` (including `+∞`) maps to the
/// half-circumference, the distance of the antipode.
pub fn squared_chord_to_metres(c2: f64) -> f64 {
    let half_chord = super::clamp_unit(c2.max(0.0).sqrt() * 0.5); // = sin(d/2)
